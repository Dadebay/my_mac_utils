...
struct SystemOverviewWidgetView: View {
    ...
    private var memoryCard: some View {
        overviewCard(symbol: "memorychip", title: s("Bellek", "Memory", "Память")) {
            ...
            HStack(spacing: 5) {
                legendDot(WidgetPalette.accent, snapshot.memory.appMemory)
    ...
    private var memoryTint: Color {
        if snapshot.memory.unreclaimableFraction >= 0.8 { return WidgetPalette.danger }
        if snapshot.memory.unreclaimableFraction >= 0.6 { return WidgetPalette.warning }
        return WidgetPalette.accent
    }
