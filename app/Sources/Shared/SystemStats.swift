...
struct MemoryStats: Equatable, Sendable {
    var total: UInt64 = 0
    /// Uygulamaların tuttuğu bellek: Etkinlik İzleyicisi'ndeki "Uygulama
    /// Belleği" ile aynı tanım (`internal - purgeable`).
    ///
    /// Eskiden burada `active_count` vardı; o yalnızca *yakın zamanda
    /// dokunulmuş* sayfaları sayıyor ve gerçek uygulama belleğinden düşük
    /// çıkıyordu. İki panel iki ayrı formül kullandığı için aynı anda
    /// farklı yüzdeler gösteriyorlardı.
    ...
    /// Boşaltılamayan pay: çivilenmiş + sıkıştırılmış sayfaların toplam
    /// içindeki oranı.
    ///
    /// **Bu macOS'un "Bellek Baskısı" değeri değildir.** Sistem gerçek
    /// baskıyı herkese açık, eşzamanlı bir yüzde olarak yayımlamıyor; bu
    /// yüzden kullanıcıya "baskı" diye gösterilmiyor. Yalnızca kartın
    /// rengini ve "çivilenmiş + sıkıştırılmış" satırını besliyor.
    var unreclaimableFraction: Double {
...
extension MemoryStats: Codable {
    private enum CodingKeys: String, CodingKey {
        case total, appMemory, wired, compressed, cached, free
        case swapUsed, swapTotal, history
        /// Eski anlık görüntülerdeki ad. Yalnızca okuma için duruyor.
        case active
    }

    /// Diskteki eski widget anlık görüntüleri `active` anahtarını taşıyor.
    /// Tek bir alan yüzünden bütün görüntünün okunamaz hâle gelmesi widget'ı
    /// boş gösterirdi; her alan tek tek, yoksa varsayılanıyla okunuyor.
    init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.init()
        total = try container.decodeIfPresent(UInt64.self, forKey: .total) ?? 0
        appMemory = try container.decodeIfPresent(UInt64.self, forKey: .appMemory)
            ?? container.decodeIfPresent(UInt64.self, forKey: .active)
            ?? 0
        wired = try container.decodeIfPresent(UInt64.self, forKey: .wired) ?? 0
        compressed = try container.decodeIfPresent(UInt64.self, forKey: .compressed) ?? 0
        cached = try container.decodeIfPresent(UInt64.self, forKey: .cached) ?? 0
        free = try container.decodeIfPresent(UInt64.self, forKey: .free) ?? 0
        swapUsed = try container.decodeIfPresent(UInt64.self, forKey: .swapUsed) ?? 0
    ...
    func encode(to encoder: any Encoder) throws {
        ...
        try container.encode(appMemory, forKey: .appMemory)
        try container.encode(wired, forKey: .wired)
        ...
        try container.encode(swapTotal, forKey: .swapTotal)
