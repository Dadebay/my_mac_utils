...
struct PanelProcessorView: View {
    ...
    private var memorySummary: some View {
        HStack(alignment: .center, spacing: 12) {
            ...
            StatSegmentedRing(
                segments: [
                    (Double(memory.appMemory) / Double(max(memory.total, 1)), SystemPalette.accent),
    ...
    private var memoryLegend: some View {
        HStack(spacing: 8) {
            memoryItem(L10n.memoryAppsLabel, memory.appMemory, SystemPalette.accent)
    ...
    private var memoryTint: Color {
        if memory.unreclaimableFraction >= 0.8 { return SystemPalette.danger }
        if memory.unreclaimableFraction >= 0.6 { return SystemPalette.warning }
        return SystemPalette.positive
    }

    private var memoryPressureText: String {
        "\(L10n.memoryUnreclaimableLabel) \(SystemFormat.percent(memory.unreclaimableFraction, decimals: 0))"
    }
}
...
struct PanelDiskView: View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let error = analyzer.errorMessage, !analyzer.items.isEmpty {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    // "Yönetici izni gerekiyor" deyip kullanıcıyı ortada
                    // bırakmak yerine iznin verildiği yere götürüyoruz:
                    // Gizlilik ve Güvenlik → Uygulama Yönetimi.
                    HStack(spacing: 10) {
                        Button(L10n.openPrivacySettings) {
                            if let url = URL(
    ...
    private func storageRow(_ item: StorageCandidate) -> some View {
        HStack(spacing: 9) {
            ...
            // Sistem uygulamalarında düğme hiç çizilmiyor: SIP silmeye
            // izin vermiyor, basıp hata almak kullanıcıyı yanıltırdı.
            if allowsDeletion, item.canDelete {
                Button {
