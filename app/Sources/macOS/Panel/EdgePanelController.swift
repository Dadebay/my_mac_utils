...
final class EdgePanelController: NSObject, NSWindowDelegate {
    ...
    func attach(container: ModelContainer, @ViewBuilder content: () -> some View) {
        ...
        // Pencere ilk andan itibaren açık panelin ölçüsünde kuruluyor.
        // Aç/kapat artık pencereyi yeniden boyutlandırmıyor (hareket
        // SwiftUI tarafında), bu yüzden dar ray ölçüsüyle kurulan pencerede
        // ilk açılışta gövdeye yer kalmıyor ve panel yanlış açılıyordu.
        // Sonraki açılışların doğru görünmesinin sebebi de buydu: pencere
        // o arada bir kez tam boyuta geçmiş oluyordu.
        ...
        hostingView.interactiveRange = { [weak self] in
            guard let self, let panel = self.panel else { return 0...CGFloat.greatestFiniteMagnitude }
            let width = panel.frame.width
            guard self.visualState != .expanded else { return 0...width }

            // Kapalıyken yalnızca rayın durduğu şerit tıklanabilir.
            let rail = CGFloat(PanelSettings.railWidth)
            return self.edge == .trailing ? (width - rail)...width : 0...rail
        }
    ...
    private func targetFrame(for state: PanelVisualState, on screen: NSScreen) -> NSRect {
        ...
        switch state {
        case .rail:
            // Ray durumunda da pencere açık boyutunda duruyor: pencere
            // büyüyüp küçüldüğü sürece içindeki ray da o harekete katılıyor
            // ve "ray sıfırdan açılıyor" görüntüsü çıkıyordu. Artık açılıp
            // kapanan şey pencere değil, SwiftUI tarafındaki panel gövdesi;
            // kapalıyken gövdenin durduğu alan saydam ve tıklamaya kapalı
            // (bkz. `FirstMouseHostingView.interactiveRange`).
    ...
    private func setVisualState(_ newState: PanelVisualState) {
        ...
        // Açılış her zaman rayın kendi konumundan başlıyor.
        //
        // Ölçüt mantıksal durum değil pencerenin **gerçek** çerçevesi:
        // kullanıcı rayı ortaya çıkarmak için fareyi kenara getirdiğinde
        // "sliver → ray" animasyonu başlıyor ve tıklama çoğu zaman o
        // bitmeden geliyor. O anda durum zaten `.rail` göründüğü için
        // duruma bakan bir denetim hiçbir şey yapmıyor, açılış da yarıda
        // kalmış — bir kısmı ekranın dışında olan — konumdan devam ediyordu:
        // ikonlar panelle birlikte sağdan içeri süzülüyordu. Buradaki
        // sıfır süreli atlama hem süren animasyonu kesiyor hem de pencereyi
        // rayın yerine oturtuyor; ardından yalnızca genişlik animasyonu
        // çalışıyor, yani açılış kapanışın aynadaki hâli oluyor.
        if newState == .expanded {
            let railTarget = targetFrame(for: .rail, on: screen)
            // Tolerans 4 pt: pencere ile hesaplanan ray çerçevesi arasında
            // yuvarlamadan gelen 1 pt'lik fark sürekli oluşuyor ve daha dar
            // bir eşik her açılışta gereksiz yere tetikleniyordu. Sıfır
            // süreli animasyon da kaldırıldı — ölçüm, onun hemen ardından
            // gelen gerçek animasyonu bozduğunu, pencerenin tek karede
            // hedefin %85'ine zıpladığını gösterdi. Düz `setFrame` süren
            // animasyonu zaten iptal ediyor.
            let offBy = max(
                abs(panel.frame.minX - railTarget.minX),
                abs(panel.frame.maxX - railTarget.maxX)
            )
            if offBy > 4 {
                panel.setFrame(railTarget, display: false)
            }
        }
        ...
        // Ray ile açık panel arasında pencere çerçevesi aynı: hareket
        // tamamen SwiftUI tarafında, panel gövdesinin genişliğinde oluyor.
        // Pencereye dokunmamak, rayın kımıldamayacağının garantisi.
        if previous != .sliver, newState != .sliver {
            PanelFrameLog.begin(
                from: previous, to: newState,
                start: panel.frame, target: target, duration: 0, panel: panel
            )
            return
        }

        // Ray genişliğini değiştiren geçişler (sliver ↔ ray) animasyonsuz.
        // İkon şeridi hiçbir zaman "açılıyor" gibi görünmemeli: kenara
        // gizlenmiş raydan geri gelirken genişleyerek büyümesi, panelin
        // açılışıyla karışıyor ve ikonlar sıfırdan açılıyormuş gibi
        // okunuyordu. Panelin kendi açılıp kapanması animasyonlu kalıyor.
        if previous == .sliver || newState == .sliver {
            panel.setFrame(target, display: true)
            PanelFrameLog.begin(
                from: previous, to: newState,
        ...
        PanelFrameLog.begin(
            from: previous, to: newState,
            start: panel.frame, target: target, duration: duration, panel: panel
        )

        NSAnimationContext.runAnimationGroup { ctx in
            ctx.duration = duration
            // Örtük animasyon KAPALI: açıkken pencere çerçevesiyle birlikte
            // içerik katmanı da kendi başına animasyon kuruyor ve büyüme
            // yönünde kayıyordu — kenara yaslı ray, katman kaydığı için
            // ekranda oynuyordu. Kapalıyken pencere yine `animator()` ile
            // yumuşak boyutlanıyor; içerik ise her karede yeni boyuta göre
            // yeniden yerleşiyor, yani ray çapasından hiç ayrılmıyor.
            ctx.allowsImplicitAnimation = false
            // Drawer eğrisi: hızlı tepki verir, sona yaklaşırken yumuşar ve
            // spring gibi ekran kenarından taşıp geri sekmez.
            //
            // Önceki eğri `(0.32, 0.72, 0, 1)` ölçümde mesafenin %80'ini
            // ilk %27'lik zamanda alıyordu; 0.26 sn'de bu, açılışı "bir
            // anda oldu" diye gösteriyor ve sabit duran ray bile o ani
            // genişlemeyle zıplamış gibi okunuyordu. Bu eğri aynı yönde
            // ama ölçülü: ilk %27'de yol yaklaşık yarıya geliyor.
            ctx.timingFunction = CAMediaTimingFunction(controlPoints: 0.25, 0, 0.2, 1)
            panel.animator().setFrame(target, display: true)
        }
    }

    // MARK: - Hareket günlüğü

    /// Panel çerçevesinin aç/kapat sırasında gerçekte nereye gittiğini
    /// kaydeder. Tahmin yerine ölçüm: hangi kenarın sabit kaldığı, hangi
    /// yönde büyüdüğü ve animasyonun nereden başladığı ancak kare kare
    /// bakınca görülüyor.
    ///
    /// Kayıt, uygulamanın kum havuzu (sandbox) geçici klasöründeki
    /// `panel-motion.log` dosyasına yazılıyor: birleşik günlükte
    /// (`log show`) satırlar görünmedi, dosya ise kesin ve okunması kolay.
    @MainActor
    enum PanelFrameLog {
        private static var sampler: Timer?

        static func begin(
            from previous: PanelVisualState, to newState: PanelVisualState,
            start: NSRect, target: NSRect, duration: Double, panel: NSWindow
        ) {
            sampler?.invalidate()

            write(
                "GEÇİŞ \(previous) → \(newState) | başlangıç \(describe(start))"
                    + " | hedef \(describe(target)) | süre \(String(format: "%.3f", duration))"
            )

            // Zamanlayıcı ana döngüde çalışıyor; `MainActor.assumeIsolated`
            // ile o gerçek derleyiciye söyleniyor — geri çağırmayı ayrı bir
            // göreve atmak hem gereksiz hem de kareleri geciktirirdi.
            let deadline = Date().addingTimeInterval(duration + 0.12)
            // Geri çağırmaya gelen `Timer` değeri Sendable değil; ona hiç
            // dokunmayıp saklanan referans üzerinden durduruluyor.
            sampler = Timer.scheduledTimer(withTimeInterval: 1.0 / 60, repeats: true) { _ in
                MainActor.assumeIsolated {
                    write("  kare \(describe(panel.frame))")
                    if Date() >= deadline {
                        sampler?.invalidate()
                        sampler = nil
                        write("BİTTİ \(describe(panel.frame))")
                    }
                }
            }
        }

        /// Dosya yolu uygulamanın geçici klasöründe; kum havuzu içinden
        /// yazılabilen ve dışarıdan okunabilen tek yer burası.
        static var logURL: URL {
            FileManager.default.temporaryDirectory.appendingPathComponent("panel-motion.log")
        }
        ...
        private static func describe(_ f: NSRect) -> String {
            String(
                format: "x=%.0f maxX=%.0f w=%.0f y=%.0f maxY=%.0f h=%.0f",
                f.minX, f.maxX, f.width, f.minY, f.maxY, f.height
            )
        }
    }
