...
public enum L10n {
    ...
    /// Sonlandırma denemesinin sonucu kullanıcıya söyleniyor: sessizce
    /// satırı listeden silmek, iş görmediğinde yalan oluyordu.
    public static func processQuitDenied(_ name: String) -> String {
        s(
            "\(name) sonlandırılamadı — bu sürece sinyal göndermeye izin yok.",
            "Couldn't quit \(name) — this process can't be signalled by GlassDo.",
            "Не удалось завершить \(name) — нет прав на сигнал этому процессу."
        )
    }
    public static func processQuitRestarted(_ name: String) -> String {
        s(
            "\(name) kapandı ama sistem onu hemen yeniden başlattı.",
            "\(name) quit, but the system restarted it right away.",
            "\(name) завершён, но система сразу запустила его заново."
        )
    }
    public static var openPrivacySettings: String {
        s("Gizlilik Ayarlarını Aç", "Open Privacy Settings", "Открыть настройки конфиденциальности")
    }
    ...
    /// Eski ad. macOS'un gerçek "Memory Pressure" değeri değildi; arayüzde
    /// artık kullanılmıyor, yalnızca eski kayıtlarla uyum için duruyor.
    public static var memoryPressureLabel: String { s("Baskı", "Pressure", "Нагрузка") }
    /// Boşaltılamayan pay: çivilenmiş + sıkıştırılmış sayfalar. Ne olduğunu
    /// olduğu gibi söylüyor, "baskı" diye bir yorum katmıyor.
    public static var memoryUnreclaimableLabel: String {
        s("Sabit + sıkıştırılmış", "Wired + compressed", "Закреплённая + сжатая")
    }
