...
/// "En Büyük Uygulamalar" listesinin sözleşmesi.
///
/// Gerçek disk taranmıyor; kural saf bir karardan ibaret olduğu için
/// uydurma yollarla sınanıyor: yalnızca `.app` aday olur, sistem
/// uygulamaları silinemez.
struct ApplicationScanRuleTests {

    private func isApplicationBundle(_ path: String) -> Bool {
        URL(fileURLWithPath: path).pathExtension.lowercased() == "app"
    }

    @Test(".app paketi uygulama sayılıyor")
    func appBundleQualifies() {
        #expect(isApplicationBundle("/Applications/Xcode.app"))
    }

    @Test("Uygulama olmayan paketler listeye girmiyor")
    func nonApplicationPackagesAreRejected() {
        #expect(!isApplicationBundle("/Users/x/Pictures/Photos.photoslibrary"))
        #expect(!isApplicationBundle("/Users/x/Dev/App.xcodeproj"))
        #expect(!isApplicationBundle("/Library/Frameworks/Sparkle.framework"))
        #expect(!isApplicationBundle("/Users/x/Movies/kayit.mov"))
    }

    @Test("Sistem uygulamasında silme sunulmuyor, kullanıcı uygulamasında sunuluyor")
    func systemApplicationsAreNotDeletable() {
        let system = StorageCandidate(
            url: URL(fileURLWithPath: "/System/Applications/Safari.app"),
            allocatedSize: 1024,
            kind: .application,
            isSystem: true
        )
        let user = StorageCandidate(
            url: URL(fileURLWithPath: "/Applications/Xcode.app"),
            allocatedSize: 1024,
            kind: .application
        )

        #expect(!system.canDelete)
        #expect(user.canDelete)
    }
}
