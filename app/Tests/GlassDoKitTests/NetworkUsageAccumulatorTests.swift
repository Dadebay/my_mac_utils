...
/// Geçmiş dosyasının App Group'a taşınmasıyla ilgili sözleşme.
///
/// Gerçek App Group kabına dokunulmuyor; göç mantığının kuralları burada
/// geçici klasörlerle sınanıyor: yeni dosya varsa korunur, bozuk eski dosya
/// yeniyi ezmez, taşınan dosya kaybolmaz.
struct NetworkHistoryMigrationTests {

    private func makeTemporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("glassdo-migration-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
        return url
    }

    private func payloadData(today: UInt64) throws -> Data {
        var accumulator = NetworkUsageAccumulator()
        // İlk örnek çizgiyi kuruyor, ikincisi farkı üretiyor.
        accumulator.ingest(
            samples: [NetworkInterfaceSample(name: "en0", received: 0, sent: 0, flags: 0)],
            at: Date()
        )
        accumulator.ingest(
            samples: [NetworkInterfaceSample(name: "en0", received: today, sent: 0, flags: 0)],
            at: Date()
        )
        return try JSONEncoder().encode(accumulator.payload)
    }

    @Test("Geçerli eski dosya yeni konuma kopyalanıyor ve okunabiliyor")
    func migratesValidLegacyPayload() throws {
        let directory = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }

        let legacy = directory.appendingPathComponent("legacy.json")
        let destination = directory.appendingPathComponent("network-history.json")
        try payloadData(today: 5_000).write(to: legacy)

        // Göçün kuralı: kaynak çözümlenebiliyorsa hedefe atomik yazılır.
        let data = try Data(contentsOf: legacy)
        let decoded = try JSONDecoder().decode(NetworkUsagePayload.self, from: data)
        try data.write(to: destination, options: .atomic)

        let reread = try JSONDecoder().decode(
            NetworkUsagePayload.self, from: Data(contentsOf: destination)
        )
        #expect(reread == decoded)
    }

    @Test("Bozuk eski dosya çözümlenemiyor, hedefi ezmiyor")
    func corruptLegacyPayloadIsRejected() throws {
        let directory = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }

        let legacy = directory.appendingPathComponent("legacy.json")
        try Data("{ bu gecerli json degil".utf8).write(to: legacy)
